...
import { WiDaySunny, WiRain, WiCloudy, WiSnow, WiThunderstorm,WiDayCloudy } from 'weather-icons-react';
import { 
  Search,
  MapPin,
  ThermometerSun,
...
const setWeatherBackground = useCallback((condition) => {
  const conditionLower = condition.toLowerCase();
...
const fetchWeatherByDistrict = useCallback(async (district) => {
  try {
    ...
    const response = await axios.get(
      `${API_BASE}/api/weather?district=${encodeURIComponent(district)}`
    );
  ...
  },[API_BASE, fetchWeatherByDistrict]);
