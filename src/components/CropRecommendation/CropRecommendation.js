...
import {
  ...
  Leaf, Thermometer, Droplets, CloudRain, Compass,
  BarChart3, Calendar, Zap, TrendingUp, Shield,
  Calculator,  AlertCircle, CheckCircle,
  Info,  Globe, RefreshCw, MapPin,  Droplet, WindIcon,
  Sunrise, Sunset, Target,
  Wifi, CloudSun, DollarSign,
  Package, Clock, ThermometerIcon, Droplet as DropletIcon,
   Layers, Sprout as SproutIcon, XCircle
} from 'lucide-react';
...
const CropRecommendation = () => {
  const [formData, setFormData] = useState({
    ...
    temperature: '25', humidity: '82', rainfall: '202',
    land_size: '1', season: 'kharif',
    latitude: '', longitude: '', use_live_weather: false
  });
...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/api/recommend/crop`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="bg-gradient-to-br from-blue-500 to-cyan-500 text-white rounded-2xl p-6 shadow-lg mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white/20 p-3 rounded-xl backdrop-blur-sm">
            ...
            <div className="text-xl font-bold mt-1">{current.humidity || 'N/A'}%</div>
          </div>
          ...
          <div className="bg-white/20 p-3 rounded-xl backdrop-blur-sm">
            ...
            <div className="text-xl font-bold mt-1">{current.wind_speed || 'N/A'} m/s</div>
          </div>
          ...
          <div className="bg-white/20 p-3 rounded-xl backdrop-blur-sm">
            ...
            <div className="text-xl font-bold mt-1">{current.sunrise || 'N/A'}</div>
          </div>
          ...
          <div className="bg-white/20 p-3 rounded-xl backdrop-blur-sm">
            ...
            <div className="text-xl font-bold mt-1">{current.sunset || 'N/A'}</div>
          </div>
        </div>
      </div>
    );
  ...
  const SustainabilityRadar = ({ scores }) => {
    const metrics = [
      { key: 'ph', label: 'Soil pH', color: 'bg-green-500', icon: '🧪', description: 'Soil acidity/alkalinity balance' },
      { key: 'nitrogen', label: 'Nitrogen', color: 'bg-blue-500', icon: '⚗️', description: 'Plant growth nutrient' },
      { key: 'rainfall', label: 'Rainfall', color: 'bg-cyan-500', icon: '🌧️', description: 'Water availability' },
      { key: 'temperature', label: 'Temperature', color: 'bg-orange-500', icon: '🌡️', description: 'Climate suitability' },
    ];
          ...
          <div className="relative">
            <div className="w-48 h-48 rounded-full border-8 border-teal-200 flex items-center justify-center">
              <div className="text-center">
                <div className="text-4xl font-bold text-teal-700">{scores.overall}</div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="text-right">
                        <div className={`text-2xl font-bold ${
                          ...
                          score >= 10 ? 'text-amber-600' : 'text-red-600'
                        }`}>
                          {score}/25
                        </div>
                        <div className={`text-sm ${
                          status === 'Optimal' ? 'text-green-700' :
                          status === 'Acceptable' ? 'text-amber-700' : 'text-red-700'
                        }`}>
                          {status}
                        </div>
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {scores.ph?.score < 20 && (
              <div className="flex items-start">
                ...
                <span className="text-sm text-gray-600">Consider adding lime (to raise pH) or sulfur (to lower pH) to optimize soil acidity.</span>
              </div>
            )}
            {scores.nitrogen?.score < 20 && (
              <div className="flex items-start">
                <AlertCircle className="h-5 w-5 text-amber-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-gray-600">Add organic compost or nitrogen-rich fertilizer to improve soil fertility.</span>
              </div>
            )}
            {scores.temperature?.score < 20 && (
              <div className="flex items-start">
                <AlertCircle className="h-5 w-5 text-amber-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-gray-600">Consider shade nets for heat protection or greenhouse cultivation for temperature control.</span>
              </div>
            )}
            {scores.overall >= 80 && (
              <div className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-gray-600">Excellent farming conditions! Your farm is well-suited for sustainable agriculture.</span>
              </div>
            )}
          </div>
        </div>
  ...
  const InteractiveInputPanel = () => (
    <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-4 mb-6 border border-blue-200">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="font-bold text-blue-800">Live Weather Integration</h3>
              <p className="text-sm text-blue-600">Get real-time weather data for accurate predictions</p>
            </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => setFormData({...formData, use_live_weather: e.target.checked})}
              ...
              <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                <div className="flex items-center text-blue-700">
                  ...
                  <span className="font-medium">
                    Current: {weatherData?.current?.temperature || 'N/A'}°C, {weatherData?.current?.weather || 'No data'}
                  </span>
                </div>
              </div>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {[
          { id: 'n', label: 'Nitrogen (N)', icon: <Zap className="text-amber-500" />, min: 0, max: 140, unit: 'ppm', optimal: [80, 120] },
          { id: 'p', label: 'Phosphorus (P)', icon: <Leaf className="text-green-500" />, min: 5, max: 145, unit: 'ppm', optimal: [40, 80] },
          { id: 'k', label: 'Potassium (K)', icon: <Shield className="text-purple-500" />, min: 5, max: 205, unit: 'ppm', optimal: [40, 80] },
          { id: 'ph', label: 'Soil pH', icon: <BarChart3 className="text-blue-500" />, min: 3, max: 10, unit: '', step: 0.1, optimal: [6.0, 7.0] },
          { id: 'temperature', label: 'Temperature', icon: <Thermometer className="text-red-500" />, min: -20, max: 50, unit: '°C', optimal: [20, 30] },
          { id: 'humidity', label: 'Humidity', icon: <Droplets className="text-cyan-500" />, min: 0, max: 100, unit: '%', optimal: [60, 85] },
          { id: 'rainfall', label: 'Rainfall', icon: <CloudRain className="text-blue-400" />, min: 0, max: 1000, unit: 'mm', optimal: [150, 300] },
          { id: 'land_size', label: 'Land Size', icon: <Compass className="text-amber-700" />, min: 0.1, max: 100, unit: 'acres', optimal: [1, 10] },
        ].map((field) => (
          <div key={field.id} className="bg-gray-50 p-4 rounded-xl border border-gray-200">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center">
                {field.icon}
                <span className="ml-2 font-medium text-gray-700">{field.label}</span>
              </div>
              <div className="text-lg font-bold text-green-700">
                {formData[field.id]}{field.unit}
              </div>
            </div>
            ...
            <div className="flex justify-between text-xs text-gray-500 mt-2">
              ...
              <span className="font-medium text-green-600">Optimal: {field.optimal?.[0]}-{field.optimal?.[1]}{field.unit}</span>
              <span>{field.max}{field.unit}</span>
            </div>
            
            {/* Visual indicator for optimal range */}
            <div className="relative h-2 bg-gray-200 rounded-full mt-2 overflow-hidden">
              <div className="absolute h-full bg-green-300 rounded"
                style={{
              ...
              <div className="absolute w-3 h-4 -mt-1 bg-blue-600 rounded-full"
                style={{ 
  ...
  const CropDetailsDisplay = ({ crop }) => {
    ...
    const getCropIcon = (cropName) => {
      ...
      if (lowerCrop.includes('maize') || lowerCrop.includes('corn')) return '🌽';
      if (lowerCrop.includes('cotton')) return '🧵';
      if (lowerCrop.includes('jute')) return '🎋';
      if (lowerCrop.includes('chickpea') || lowerCrop.includes('bean')) return '🫘';
      ...
      if (lowerCrop.includes('watermelon') || lowerCrop.includes('melon')) return '🍈';
      ...
      <div className="bg-white rounded-2xl p-6 border border-green-200 mb-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-2xl font-bold text-green-800 capitalize">{crop.name}</h2>
              <p className="text-green-600">{crop.description || 'Suitable crop for your conditions'}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-green-50 p-4 rounded-xl border border-green-100">
            <div className="flex items-center mb-2">
              ...
              <h3 className="font-semibold text-green-700">Ideal Temperature</h3>
            </div>
            <p className="text-2xl font-bold text-green-800">{crop.ideal_temp || '20-30°C'}</p>
          ...
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
            ...
            <p className="text-2xl font-bold text-blue-800">{crop.water_needs || 'Moderate'}</p>
          ...
          <div className="bg-amber-50 p-4 rounded-xl border border-amber-100">
            ...
            <p className="text-2xl font-bold text-amber-800">{crop.growth_period || '90-120 days'}</p>
          ...
          <div className="bg-purple-50 p-4 rounded-xl border border-purple-100">
            ...
            <p className="text-2xl font-bold text-purple-800">{crop.ideal_ph || '6.0-7.0'}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-50 p-4 rounded-xl">
            ...
            <p className="text-gray-800">{crop.market_price || '₹1500-2500/quintal'}</p>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-xl">
            ...
            <p className="text-gray-800">{crop.soil_type || 'Well-drained loamy soil'}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-xl p-5 border border-gray-200 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {result.top_crops.slice(1).map((crop, idx) => (
            <div key={idx} className="bg-green-50 p-3 rounded-lg border border-green-100">
              <div className="font-medium text-green-800 capitalize">{crop.crop}</div>
              <div className="text-sm text-green-600">{crop.confidence}% match</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white/20 p-4 rounded-xl backdrop-blur-sm">
              ...
              <p className="text-3xl font-bold">
                ...
                {result.yield_analysis?.estimated_profit?.toLocaleString('en-IN') || '4,000'}
              </p>
              <p className="text-green-100 text-sm mt-1">
                Market price: {result.crop_details?.market_price || '₹2000/quintal'}
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white p-4 rounded-xl border border-gray-200 text-center">
            <div className="text-2xl font-bold text-green-700">{result.input_summary?.nutrients?.nitrogen || '90'}</div>
            <div className="text-sm text-gray-600">Nitrogen (ppm)</div>
          </div>
          
          <div className="bg-white p-4 rounded-xl border border-gray-200 text-center">
            <div className="text-2xl font-bold text-green-700">{result.input_summary?.environment?.ph || '6.5'}</div>
            <div className="text-sm text-gray-600">Soil pH</div>
          </div>
          
          <div className="bg-white p-4 rounded-xl border border-gray-200 text-center">
            <div className="text-2xl font-bold text-green-700">{result.input_summary?.environment?.temperature || '25'}°C</div>
            <div className="text-sm text-gray-600">Temperature</div>
          </div>
          
          <div className="bg-white p-4 rounded-xl border border-gray-200 text-center">
            <div className="text-2xl font-bold text-green-700">{result.input_summary?.environment?.rainfall || '202'} mm</div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <div className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-gray-700">Test soil nutrients every season before planting</span>
              </div>
              <div className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-gray-700">Maintain proper irrigation schedule based on growth stage</span>
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-gray-700">Use organic compost to improve soil health</span>
              </div>
              <div className="flex items-start">
                <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                <span className="text-sm text-gray-700">Monitor for pests regularly and use integrated pest management</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center my-8">
          ...
          <p className="text-green-600 max-w-3xl mx-auto text-lg">
            Interactive AI system that explains decisions visually for better understanding
          </p>
        </div>
        ...
        {activeTab === 'ai-explain' && result && (
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl p-6">
              <h2 className="text-2xl font-bold mb-2">AI Decision Explained Visually</h2>
              <p className="text-purple-100">See how the AI analyzed your farm data to make this recommendation</p>
            </div>
            ...
            <div className="bg-gradient-to-r from-green-50 to-teal-50 border border-green-200 rounded-xl p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-semibold text-green-700 mb-2">Traditional Systems:</h4>
                  <ul className="space-y-1 text-green-600">
                    <li className="flex items-start">
                      <span className="inline-block w-1.5 h-1.5 bg-red-400 rounded-full mt-1 mr-2"></span>
                      Black-box decisions (no explanation)
                    </li>
                    <li className="flex items-start">
                      <span className="inline-block w-1.5 h-1.5 bg-red-400 rounded-full mt=1 mr-2"></span>
                      Farmers don't know why crops are recommended
                    </li>
                    <li className="flex items-start">
                      <span className="inline-block w-1.5 h-1.5 bg-red-400 rounded-full mt=1 mr-2"></span>
                      Low trust and adoption rates
                    </li>
                  </ul>
                </div>
                <div>
                  <h4 className="font-semibold text-teal-700 mb-2">Our System:</h4>
                  <ul className="space-y-1 text-teal-600">
                    <li className="flex items-start">
                      <span className="inline-block w-1.5 h-1.5 bg-green-500 rounded-full mt=1 mr-2"></span>
                      Transparent SHAP-based explanations
                    </li>
                    <li className="flex items-start">
                      <span className="inline-block w-1.5 h-1.5 bg-green-500 rounded-full mt=1 mr-2"></span>
                      Visual impact analysis for each factor
                    </li>
                    <li className="flex items-start">
                      <span className="inline-block w-1.5 h-1.5 bg-green-500 rounded-full mt=1 mr-2"></span>
                      Increased farmer trust and understanding
                    </li>
                  </ul>
                </div>
              </div>
        ...
        {activeTab === 'sustainability' && result && (
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-2xl p-6">
              <h2 className="text-2xl font-bold mb-2">Farm Sustainability Analysis</h2>
              <p className="text-teal-100">Environmental impact assessment of your farm conditions</p>
            </div>
            
            {result.sustainability && <SustainabilityRadar scores={result.sustainability} />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {result.sustainability_explanations.map((exp, idx) => (
                    <div key={idx} className={`p-4 rounded-lg ${
                      exp.includes('✅') || exp.includes('🌟') ? 'bg-green-50 border border-green-200' :
                      exp.includes('⚠️') ? 'bg-yellow-50 border border-yellow-200' :
                      exp.includes('❌') ? 'bg-red-50 border border-red-200' :
                      ...
                      <div className="flex items-start">
                        ...
                        <p className="text-gray-700">{exp.replace(/[✅🌟⚠️❌]/g, '').trim()}</p>
                      </div>
                    </div>
                  ))}
                </div>
